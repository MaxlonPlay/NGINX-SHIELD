...
class TokenManager:
        ...
        try:
            ...
            debug_logger.token(
                "Token decodificato con successo",
                {
                    ...
                    "expires_in_seconds": payload.get("exp", 0)
                    - datetime.utcnow().timestamp(),
                },
            )
            return payload
        except JWTError as e:
            debug_logger.token("Errore nella decodifica token", {"error": str(e)})
            return None
    ...
    def set_token_cookie(
        self, response: Response, token: str, max_age: int = None
    ) -> None:
        if max_age is None:
    ...
    def validate_and_refresh_token(
        ...
        if username is None or exp is None:
            debug_logger.token(
                "Payload incompleto - 401", {"username": username, "exp": exp}
            )
            raise credentials_exception
        ...
        if seconds_left < 30:
            debug_logger.token(
                "Token prossimo alla scadenza, emetto nuovo token",
            ...
            debug_logger.token(
                "Token valido", {"username": username, "seconds_left": seconds_left}
            )
    ...
    def check_token_status(self, request: Request) -> Dict[str, Any]:
        ...
        if exp and exp > now:
            debug_logger.token(
                "Token valido nei cookie",
                {"username": payload.get("sub"), "expires_in_seconds": exp - now},
            )
            return {
                "authenticated": True,
                "username": payload.get("sub"),
...
def get_current_user_and_refresh_token(
    ...
    return token_manager.validate_and_refresh_token(request, response, credentials)
