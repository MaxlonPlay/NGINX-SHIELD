...
class BanManager:
    ...
    def _load_jail_name(self) -> str:
        ...
        try:
            ...
            self .debug_log(
                f"Errore nella lettura del file di configurazione: {e}")
            return "npm-docker"
    ...
    def get_ip_info(self, ip: str) -> Dict[str, Any]:
        try:
            ...
            if response .status_code == 200:
                ...
                self .debug_log(
                    f"Errore HTTP {
                        response .status_code} dal servizio geolocalizzazione per IP {ip}.")
        except requests .exceptions .Timeout:
            self .debug_log(
                f"Timeout nella richiesta di geolocalizzazione per IP {ip}.")
        except requests .exceptions .RequestException as e:
            self .debug_log(
                f"Errore nella richiesta di geolocalizzazione per IP {ip}: {e}")
        except Exception as e:
            self .debug_log(
                f"Errore generico nella geolocalizzazione per IP {ip}: {e}")
    ...
    def setup_db(self) -> bool:
        try:
            ...
            self .debug_log(
                f"Database SQLite '{
                    self .db_file}' creato/aggiornato con successo.")
            return True
        except Exception as e:
            self .debug_log(
                f"Errore nella creazione/aggiornamento del database: {e}")
            return False
    ...
    def is_ip_banned_in_db(
            self, ip: str) -> Tuple[bool, Optional[str], Optional[str]]:
        ...
        try:
            ...
            c .execute(
                "SELECT ip, domain FROM automatic_bans WHERE ip = ?", (ip,))
            auto_result = c .fetchone()
            if auto_result:
                conn .close()
                return True, "automatic", f"Ban automatico per dominio: {
                    auto_result[1] or 'N/A'}"

            c .execute(
                "SELECT ip, reason FROM manual_bans WHERE ip = ?", (ip,))
    ...
    def ban_ip_manual(self, ip: str, reason: str) -> Dict[str, Any]:
        ...
        try:
            ...
            c .execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (ip, reason, timestamp, ip_info['network'], ip_info['asn'],
            ...
            self .debug_log(
                f"Ban manuale per IP {ip} (motivo: {reason}) aggiunto alla tabella 'manual_bans' con info geo: {
            ...
            self .debug_log(
                f"Errore nell'aggiunta del ban manuale per IP {ip}: {e}")
            return {
                "success": False,
                "message": f"Errore durante il salvataggio nel database: {
                    str(e)}",
    ...
    def unban_ip(self, ip: str, ban_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not success:
                self .debug_log(
                    f"Avviso: Errore rimozione da fail2ban: {output}")
            ...
            self .debug_log(
                f"IP/CIDR {ip} rimosso dai ban {ban_type} con successo")
    ...
    def get_banned_ips(self, limit: int = 100, automatic_offset: int = 0,
                       manual_offset: int = 0, search_query: str = "",
                       export_mode: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if not export_mode:
                ...
                auto_params = search_params_auto + \
                    [limit, automatic_offset]if search_query else [limit, automatic_offset]
            else:
            ...
            if not export_mode:
                ...
                manual_params = search_params_manual + \
                    [limit, manual_offset]if search_query else [limit, manual_offset]
            else:
            ...
            for row in auto_results:
                automatic_bans .append({
                    ...
                    "domain": row[2],
                    "userAgent": row[3],
                    "httpCode": int(row[4])if row[4] and str(row[4]).isdigit()else None,
                    "urlPath": row[5],
                    "network": row[6],
                    "asn": row[7],
                    "organization": row[8],
            ...
            has_more_automatic = (
                automatic_offset +
                limit) < auto_total if not export_mode else False
            has_more_manual = (
                manual_offset +
                limit) < manual_total if not export_mode else False
    ...
    def get_ban_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            c .execute("""
                ...
                LIMIT 5
            """)
            top_domains = [{"domain": row[0], "count": row[1]}
            ...
            """)
            top_countries = [{"country": row[0], "count": row[1]}
                             for row in c .fetchall()]
            ...
            return {
                "success": False,
                "message": f"Errore durante il recupero delle statistiche: {
                    str(e)}",
                "error_type": "database_error"}
    ...
    def bulk_ban_ips_manual(
            self, ban_requests: List[Dict[str, str]]) -> Dict[str, Any]:
