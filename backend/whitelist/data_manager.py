...
class WhitelistFileManager:
    ...
    def _get_db_connection(self):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Errore di connessione al database: {
                    str(e)}")
        finally:
            if conn:
                conn .close()
    ...
    def _ensure_db_exists(self):
        with self ._get_db_connection()as conn:
            ...
            cursor .execute(
                "SELECT COUNT(*) FROM metadata WHERE key = 'version'")
            if cursor .fetchone()[0] == 0:
                now = datetime .utcnow().isoformat()
                cursor .execute(
                    "INSERT INTO metadata (key, value) VALUES (?, ?)", ('created', now))
                cursor .execute(
                    "INSERT INTO metadata (key, value) VALUES (?, ?)", ('last_modified', now))
                cursor .execute(
    ...
    def load_whitelist(self) -> Dict[str, Any]:
        with self ._get_db_connection()as conn:
            ...
            cursor .execute(
                "SELECT type, value, description, created FROM entries")
    ...
    def save_whitelist(self, data: Dict[str, Any]):
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Dati di input non validi: {
                    e .errors()}")
        except TypeError as e:
            raise HTTPException(
                status_code=400,
                detail=f"Formato dati JSON non valido: {
                    str(e)}")
        ...
        with self ._get_db_connection()as conn:
            ...
            entries_to_add = []
            values_in_new_data = {
                entry .value for entry in validated_data .entries}

            for entry in validated_data .entries:
                if entry .value not in existing_values:
                    created_timestamp = entry .created if entry .created else datetime .utcnow().isoformat()
            ...
            now = datetime .utcnow().isoformat()
            cursor .execute(
                "REPLACE INTO metadata (key, value) VALUES (?, ?)", ('last_modified', now))
