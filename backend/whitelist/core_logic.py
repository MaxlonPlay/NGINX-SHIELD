...
class WhitelistCoreLogic:
    ...
    def add_entry(self, entry: WhitelistEntry) -> Dict[str, Any]:
        ...
        for existing_entry in data["entries"]:
            if (
                ...
                and existing_entry["value"] == entry.value
            ):
                raise HTTPException(
                    status_code=400,
    ...
    def remove_entry(self, entry_type: str, value: str) -> Dict[str, Any]:
        ...
        if len(data["entries"]) == original_count:
            raise HTTPException(
                status_code=404, detail=f"Entry non trovata: {entry_type} {value}"
            )

        self.file_manager.save_whitelist(data)

        return {"success": True, "message": "Entry rimossa con successo"}

    def update_entry(
        self, entry_type: str, value: str, new_description: str
    ) -> Dict[str, Any]:
        data = self.file_manager.load_whitelist()
        ...
        return {"success": True, "message": "Entry aggiornata con successo"}
    ...
    def search_entries(self, query: str) -> List[Dict[str, Any]]:
        ...
        for entry in entries:
            if (
                ...
                or query_lower in entry["type"].lower()
            ):
