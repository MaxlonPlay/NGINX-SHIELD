...
class DBCredentials (Base):
    ...
    def __repr__(self):
        return f"<DBCredentials(username='{
            self .username}', requires_password_change={
            self .requires_password_change}, totp_enabled={
            self .totp_enabled})>"
...
class AuthManager:
    ...
    def verify_credentials(self, username: str,
        ...
        with self .get_db_session()as db:
            try:
                ...
                stored_credentials = db .query(DBCredentials).filter(
                    DBCredentials .username == username
                ).first()

                if not stored_credentials:
                    if (username == self .default_username and
                        password == self .default_password and
                            self .is_first_login()):
                        debug_logger .auth(
                    ...
                    if not self .verify_password(
                        ...
                        debug_logger .auth(
                            "Fallimento: password non valida", {
                                "username": username})
                        return {
                            "authenticated": False,
                            "requires_password_change": False,
                    ...
                    debug_logger .auth(
                        "Password verificata", {
                            "username": username, "totp_enabled": stored_credentials .totp_enabled})

                    if stored_credentials .totp_enabled:
                        debug_logger .auth(
                            "TOTP abilitato - richiesta verifica in secondo passaggio", {"username": username})
                        return {
                            "authenticated": False,
    ...
    def setup_totp(self, username: str,
                   current_password: str) -> Dict[str, Any]:

        with self .get_db_session()as db:
            try:
                print(f"[SETUP_TOTP_AUTH] Cercando utente: {username}")
                user = db .query(DBCredentials).filter(
                    DBCredentials .username == username).first()
                ...
                print(
                    f"[SETUP_TOTP_AUTH] QR base64 creato: {
                        len(qr_base64)} bytes")
                ...
                return {
                    "success": False,
                    "message": f"Errore interno durante la configurazione TOTP: {
                        str(e)}"}

    def confirm_totp_setup(self,
                           username: str,
                           current_password: Optional[str],
                           totp_code: str) -> Dict[str,
                                                   Any]:
        ...
        with self .get_db_session()as db:
            try:
                ...
                user = db .query(DBCredentials).filter(
                    DBCredentials .username == username).first()
                print(
                    f"[CONFIRM_TOTP_AUTH] Utente trovato: {
                        user is not None}")
                ...
                setup_session = totp_session_manager .get_setup_session(
                    username)

                if not setup_session:
                    print(
                        f"[CONFIRM_TOTP_AUTH] Setup session non trovata o scaduta!")
                    return {
                        "success": False,
                ...
                print(
                    f"[CONFIRM_TOTP_AUTH] Verificando codice TOTP: {totp_code}")
                ...
                print(
                    f"[CONFIRM_TOTP_AUTH] Codice TOTP valido! Salvando nel DB...")
                ...
                print(
                    f"[CONFIRM_TOTP_AUTH] ECCEZIONE: {
                        type(e).__name__}: {e}")
                import traceback
                traceback .print_exc()
                return {
                    "success": False,
                    "message": f"Errore interno durante la conferma TOTP: {
                        str(e)}"}
                return {
                    "success": False,
                    "message": "Errore interno durante la conferma TOTP"}

    def disable_totp(self, username: str, current_password: str,
        ...
        with self .get_db_session()as db:
            try:
                ...
                return {
                    ...
                    "activated_at": user .totp_activated_at .isoformat()if user .totp_activated_at else None,
    ...
    def verify_totp_code(self, username: str,
                         totp_code: str) -> Dict[str, Any]:

        with self .get_db_session()as db:
            try:
                print(
                    f"[VERIFY_TOTP_CODE] Verificando codice TOTP per {username}")
                user = db .query(DBCredentials).filter(
                    DBCredentials .username == username).first()
                ...
                if not totp .verify(totp_code, valid_window=1):
                    print(
                        f"[VERIFY_TOTP_CODE] Codice TOTP non valido per {username}")
                    return {
                        "success": False,
                        "message": "Codice TOTP non valido"}
                ...
                return {
                    "success": False,
                    "message": f"Errore durante la verifica TOTP: {
                        str(e)}"}

    def verify_backup_codes(self, username: str,
                            provided_codes: list) -> Dict[str, Any]:

        with self .get_db_session()as db:
            try:
                print(
                    f"[VERIFY_BACKUP_CODES] Verificando {
                        len(provided_codes)} codici per {username}")
                ...
                try:
                    decrypted_codes_str = self .decrypt_sensitive_data(
                        user .backup_codes)
                    stored_codes = decrypted_codes_str .split(',')
                except Exception as e:
                    print(
                        f"[VERIFY_BACKUP_CODES] Errore nella decrittazione: {e}")
                    return {"success": False,
                ...
                if len(provided_codes) != 10:
                    return {
                        "success": False,
                        "message": f"Sono richiesti esattamente 10 codici, ne hai forniti {
                            len(provided_codes)}"}

                cleaned_provided = [code .strip().upper()
                                    for code in provided_codes]
                cleaned_stored = [code .strip().upper()
                                  for code in stored_codes]

                for i, provided_code in enumerate(cleaned_provided):
                    if provided_code not in cleaned_stored:
                        print(
                            f"[VERIFY_BACKUP_CODES] Codice {
                                i + 1} non valido: {provided_code}")
                        return {
                            "success": False,
                            "message": f"Codice {
                                i + 1} non valido"}
                ...
                new_password = ''.join(
                    random .choices(
                        ...
                        k=12))
                print(
                    f"[VERIFY_BACKUP_CODES] Nuova password generata: {new_password}")
                ...
                return {
                    "success": False,
                    "message": f"Errore durante la verifica dei codici: {
                        str(e)}"}
    ...
    def update_credentials(self,
                           ...
                           current_password: str,
                           new_username: str,
                           new_password: str) -> Dict[str,
                                                      Any]:
        with self .get_db_session()as db:
            user = db .query(DBCredentials).filter(
                DBCredentials .username == old_username).first()

            if not user:
                if (old_username == self .default_username and
                    current_password == self .default_password and
                        self .is_first_login()):
                    ...
                    if db .query(DBCredentials).filter(
                            DBCredentials .username == new_username).first():
                        return {"success": False,
    ...
    def authenticate_custom(
            ...
            username: str,
            password: str,
            totp_code: Optional[str] = None) -> bool:
    ...
    def get_user_info(self, username: str) -> Optional[Dict[str, Any]]:
        with self .get_db_session()as db:
            ...
            if user:
                return {
                    "username": user .username,
                    "last_password_update": user .updated_at .isoformat()if user .updated_at else None,
                    "requires_password_change": user .requires_password_change,
                    "totp_enabled": user .totp_enabled,
                    "totp_activated_at": user .totp_activated_at .isoformat()if user .totp_activated_at else None}
            return None
