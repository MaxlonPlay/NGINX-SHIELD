...
class SystemMonitor:
    def __init__(self):
        ...
        logging.warning(f"SystemMonitor avviato - intervallo: {LOG_INTERVAL_SECONDS}s")
    ...
    def _get_load_average(self) -> Dict[str, float]:
        ...
        try:
            ...
            data = {
                ...
                "normalized_1min": (
                    round((load1 / cpu_cores) * 100, 1) if cpu_cores > 0 else 0
                ),
            }
            self._loadavg_cache = (now, data)
            return data
        except BaseException:
            return {"1min": 0.0, "5min": 0.0, "15min": 0.0, "normalized_1min": 0.0}
    ...
    def _get_cpu_temperature(self) -> float:
        ...
        try:
            ...
            if self._temp_sensor_key and self._temp_sensor_key in temps:
                temp_value = max(
                    entry.current
                    for entry in temps[self._temp_sensor_key]
                    if entry.current
                )
    ...
    def _validate_csv_record(self, record: Dict[str, str]) -> bool:
        try:
            if not all(k in record for k in ["timestamp", "cpuUsage", "ramUsage"]):
                return False
    ...
    def _save_status_to_file(self, data: Dict[str, Any]):
        ...
        if not (0 <= record["cpuUsage"] <= 100 and 0 <= record["ramUsage"] <= 100):
            return
    ...
    def _cleanup_old_logs(self):
        ...
        try:
            ...
            with open(LOG_FILE_PATH, "r", newline="") as infile, open(
                temp_file, "w", newline=""
            ) as outfile:
                ...
                for row in reader:
                    try:
                        if datetime.fromisoformat(row["timestamp"]) >= cutoff_date:
                            writer.writerow(row)
                            records_kept += 1
                    except BaseException:
                        continue
    ...
    def get_historical_data(self, timeframe: str) -> Dict[str, Any]:
        try:
            mapping = {"24h": 86400, "1w": 604800, "1m": 2592000, "1y": 31536000}
            ...
            with safe_file_operation(LOG_FILE_PATH, "r") as csvfile:
                ...
                for row in reader:
                    try:
                        ...
                        if datetime.fromisoformat(row["timestamp"]) >= limit_datetime:
