...
class ValidationUtils:
    ...
    def validate_password_requirements(password: str) -> Dict[str, Any]:
        ...
        if errors:
            return {
                "valid": False,
                "message": f"Password non valida. Requisiti mancanti: {
                    ', '.join(errors)}"}
...
class ResponseManager:
    ...
    def create_auth_error_response(
            message: str = "Accesso non autorizzato") -> HTTPException:
...
class DatabaseUtils:
    ...
    def safe_database_operation(
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Errore interno del server durante {
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                LogManager .log_operation(
                    f"{operation_name} completato", username)
                return result
