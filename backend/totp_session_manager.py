...
class TOTPSessionManager:
    ...
    def create_setup_session(self, username: str, secret: str, qr_code: str) -> None:
        with self.lock:
            ...
            print(
                f"[TOTP_SESSION] Setup session creata per {username}, scade tra {self .SETUP_TIMEOUT_SECONDS}s"
            )

    def get_setup_session(self, username: str) -> Optional[Dict[str, str]]:
        with self.lock:
            if username not in self.sessions:
                print(f"[TOTP_SESSION] Setup session non trovata per {username}")
                return None
            ...
            print(
                f"[TOTP_SESSION] Setup session trovata per {username}, valida per altri {self .SETUP_TIMEOUT_SECONDS - (now - session['timestamp']):.0f}s"
            )
            return session
    ...
    def cancel_setup_session(self, username: str) -> bool:
        with self.lock:
            if username in self.sessions:
                del self.sessions[username]
                print(f"[TOTP_SESSION] Setup session cancellata per {username}")
                return True
            return False
