...
from .auth_manager import auth_manager, LoginRequest, PasswordChangeRequest, TOTPSetupRequest
...
async def startup_event():
    ...
    if setup_success:
        log_manager .log_operation(
            "Database inizializzato con successo", "system")
    else:
        log_manager .log_operation(
            "Errore nell'inizializzazione del database", "system")
...
def get_banned_ips(
        ...
        ge=1,
        le=100,
        description="Numero massimo di risultati per tipo"),
        automatic_offset: int = Query(
        0,
        ge=0,
        description="Offset per ban automatici"),
        manual_offset: int = Query(
        0,
        ge=0,
        description="Offset per ban manuali"),
    ...
    if not result["success"]:
        ...
        raise HTTPException(status_code=500, detail=result .get(
            "message", "Errore interno durante il recupero dei ban"))

    log_manager .log_operation(
        "Lista IP bannati recuperata",
        current_user .get('username'),
        f"Limit: {limit}, Auto offset: {automatic_offset}, Manual offset: {manual_offset}, Search: '{
            search or 'N/A'}'")

    return response_manager .create_success_response(
        data=result["data"],
        message="Lista IP bannati recuperata con successo"
    )
...
def ban_ip_manual(
    ...
    if not result["success"]:
        log_manager .log_operation(
            ...
            current_user .get('username'),
            f"IP/CIDR: {
                ban_request .ip}, Motivo: {
                ban_request .reason}, Errore: {
                result .get('message')}")
...
def unban_ip(ip: str = Query(...,
                             description="Indirizzo IP o CIDR da sbannare"),
             ban_type: str = Query(...,
                                   regex="^(automatic|manual)$",
                                   description="Tipo di ban da rimuovere"),
...
def get_ban_counts(
    ...
    log_manager .log_operation(
        ...
        current_user .get('username'),
        f"Automatici: {
            counts['total_automatic_bans']}, Manuali: {
            counts['total_manual_bans']}")

    return response_manager .create_success_response(
        data=counts,
        message="Conteggi recuperati con successo"
    )


@app .get("/api/bans/fail2ban-status",
          summary="🔒 Stato fail2ban", tags=["Bans"])
...
def get_fail2ban_status(
    ...
    log_manager .log_operation(
        "Stato fail2ban recuperato",
        current_user .get('username')
    )

    return response_manager .create_success_response(
        data=result["data"],
        message="Stato fail2ban recuperato con successo"
    )


@app .get("/api/bans/check/{ip}",
          summary="🔒 Verifica stato ban IP",
          tags=["Bans"])
...
def check_ip_ban_status(
    ...
    return response_manager .create_success_response(
        data=status_info,
        message=f"Stato ban per IP {ip} verificato"
    )


@app .get("/api/bans/geo-info/{ip}",
          summary="🔒 Info geolocalizzazione IP",
          tags=["Bans"])
...
def get_ip_geo_info(
    ...
    return response_manager .create_success_response(
        data=geo_info,
        message=f"Informazioni geolocalizzazione per IP {ip} recuperate"
    )


@app .post("/api/bans/bulk-manual",
           summary="🔒 Ban multipli manuali",
           tags=["Bans"])
...
def bulk_ban_ips_manual(
    ...
    if len(ban_requests) > 10:
        raise HTTPException(status_code=400,
                            detail="Massimo 10 IP per richiesta bulk")
    ...
    for ban_request in ban_requests:
        result = ban_manager .ban_ip_manual(
            ban_request .ip, ban_request .reason)
        results .append({
            "ip": ban_request .ip,
    ...
    log_manager .log_operation(
        ...
        current_user .get('username'),
        f"Totale: {
            len(ban_requests)}, Successo: {success_count}, Falliti: {failed_count}")
...
@app .post("/api/bans/cidr/check-ips",
           summary="🔒 Trova IP in CIDR", tags=["Bans"])
...
@app .post("/api/bans/cidr/unban-ips",
           summary="🔒 Sbanna IP in CIDR",
           tags=["Bans"])
...
def unban_ips_in_cidr(
    ...
    if not cidr or not ip_ids:
        raise HTTPException(status_code=400,
                            detail="CIDR e ip_ids sono obbligatori")
...
@app .post("/api/bans/cidr/ban-multiple",
           summary="🔒 Ban multipli CIDR", tags=["Bans"])
...
def ban_multiple_cidrs(
    ...
    if not cidr_list or len(cidr_list) == 0:
        raise HTTPException(
            status_code=400,
            detail="Lista CIDR è obbligatoria")

    if len(cidr_list) > 1000:
        raise HTTPException(status_code=400,
                            detail="Massimo 10 CIDR per richiesta")
    ...
    log_manager .log_operation(
        ...
        f"Totale: {
            result['successful'] +
            result['failed']}, Successo: {
            result['successful']}, Falliti: {
            result['failed']}")
...
def export_banned_ips(
    ...
    if format == "csv":
        ...
        writer .writerow(["IP", "Tipo", "Timestamp", "Motivo",
                          "Dominio", "Organizzazione", "Paese"])
        ...
        return Response(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": "attachment; filename=banned_ips.csv"})
...
def login(login_request: LoginRequest, response: Response):
    ...
    if not auth_result["authenticated"]:
        ...
        raise HTTPException(
            status_code=401,
            detail=auth_result .get("error", "Credenziali non valide")
        )
...
def verify_backup_codes_login(
        request: 'TOTPLoginVerifyBackupCodesRequest',
        response: Response):
    """Verifica i codici di backup e completa il login (accesso di recupero)"""

    result = auth_manager .verify_backup_codes(
        request .username,
    ...
    return response_manager .create_success_response(
        ...
        extra_fields={
            ...
            "requires_password_change": result .get(
...
def auth_status():
    ...
    log_manager .log_operation(
        "Richiesta stato autenticazione",
        details=f"Stato: {
...
@app .post("/api/totp/setup", response_model=TOTPSetupResponse,
           summary="🔒 Configura TOTP", tags=["TOTP"])
async def setup_totp(
    ...
    try:
        ...
        try:
            ...
            username = current_user .get('username')
            debug_logger .totp(
                "Username estratto dal token JWT", {
                    "username": username})
        except Exception as e:
            debug_logger .totp(
                f"Token non valido, cerco username nel body: {
                    str(e)[
                        :100]}")
        ...
        if not username:
            if request_data .username:
                ...
                debug_logger .totp(
                    "Username estratto dal request body", {
                        "username": username})
            else:
                debug_logger .totp(
        ...
        debug_logger .totp(
            "Inizio configurazione TOTP", {
                "username": username})

        result = auth_manager .setup_totp(
            username, request_data .current_password)

        if not result["success"]:
            debug_logger .totp(
                f"Setup TOTP fallito: {
                    result .get('message')}", {
                    "username": username})
            log_manager .log_operation(
        ...
        log_manager .log_operation(
            "TOTP configurato - in attesa di conferma",
            username
        )
...
@app .post("/api/totp/confirm",
           summary="🔒 Conferma configurazione TOTP",
           tags=["TOTP"])
async def confirm_totp_setup(
    ...
    try:
        ...
        result = auth_manager .confirm_totp_setup(
            username,
            None,
...
async def disable_totp(
    ...
    if not result["success"]:
        debug_logger .totp(
            f"Disabilitazione TOTP fallita: {
                result .get('message')}", {
                "username": username})
        log_manager .log_operation(
    ...
    log_manager .log_operation(
        "TOTP disabilitato con successo",
        username
    )
...
async def get_totp_status(
    ...
    from .backend_debug_logger import debug_logger
    debug_logger .totp("TOTP status recuperato", {
        "username": username,
...
@app .post("/api/totp/regenerate-backup-codes",
...
async def regenerate_backup_codes(
    ...
    if current_username != target_username and current_user .get(
            'role') != 'admin':
        log_manager .log_operation(
    ...
    log_manager .log_operation(
        ...
        target_username,
        f"Rigenerati da {current_username}"if current_username != target_username else "Autorigenerati"
    )
    ...
    return {
        ...
        "message": result .get(
            "message",
            "Codici di backup rigenerati con successo"),
        "backup_codes": result .get(
            "backup_codes",
            [])}
...
def update_credentials(
    ...
    effective_username_to_update = auth_manager .default_username if is_first else username_from_token
    ...
    token_manager .set_token_cookie(
        response,
        new_access_token,
        token_manager .access_token_expire_minutes * 60)
...
def verify_credentials(
    ...
    if not auth_manager .authenticate_custom(creds .username, creds .password):
        log_manager .log_auth_event(
            ...
            creds .username,
            success=False,
            details=f"Richiesta da utente JWT: '{
                current_user .get('username')}'")
        raise HTTPException(
            status_code=401,
            detail="Accesso non autorizzato: credenziali non valide")
...
@app .get("/api/whitelist/entries",
          summary="🔒 Lists entries whitelist",
          tags=["Whitelist"])
...
def get_whitelist_entries(
    ...
    entries = whitelist_manager .search_entries(
        search)if search else whitelist_manager .get_entries()
...
@app .delete("/api/whitelist/entries",
             summary="🔒 Remove entry whitelist",
             tags=["Whitelist"])
...
def remove_whitelist_entry(
    ...
    if not result["success"]:
        ...
        raise HTTPException(status_code=404, detail=result .get(
            "message", "Entry non trovata o impossibile rimuovere"))
...
@app .put("/api/whitelist/entries",
          summary="🔒 Update entry whitelist",
          tags=["Whitelist"])
...
def update_whitelist_entry(
    ...
    result = whitelist_manager .update_entry(
        entry .type, entry .value, entry .description)
    if not result["success"]:
        log_manager .log_operation(
            ...
            f"Tipo='{
                ...
                result .get(
                    ...
                    'Errore sconosciuto')}")
        raise HTTPException(status_code=404, detail=result .get(
            "message", "Entry non trovata o impossibile aggiornare"))
...
@app .get("/api/whitelist/stats",
          summary="🔒 Get whitelist statistics",
          tags=["Whitelist"])
...
def get_whitelist_stats(
    ...
    log_manager .log_operation(
        "Richiesta statistiche whitelist",
        current_user .get('username'))

    return response_manager .create_success_response(
        extra_fields={"stats": stats}
    )


@app .get("/api/whitelist/metadata",
          summary="🔒 Get whitelist metadata",
          tags=["Whitelist"])
...
async def update_system_config(
    ...
    log_manager .log_operation(
        "Tentativo aggiornamento configurazione",
        current_user .get('username'))
    ...
    return JSONResponse(
        content={"message": "Configurazione aggiornata con successo."},
        status_code=200
    )
...
def get_system_status(
    ...
    return response_manager .create_success_response(
        data=status_data,
        message="Stato del sistema recuperato con successo."
    )


@app .get("/api/system/history",
          summary="📊 Storico del sistema",
          tags=["System"])
...
def get_system_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore lettura file storico: {
                str(e)}")
...
def health_check():
    ...
    try:
        ...
        secure_cookies_enabled = os .getenv(
            "SECURE_COOKIES", "true").lower() == "true"

    log_manager .log_operation(
        "Health check richiesto",
        details="Servizio NGINX Shield: healthy")
...
@app .get("/api/mail/config",
          summary="🔒 Recupera configurazione email",
          tags=["Mail"])
...
def get_mail_config(
    ...
    try:
        ...
        log_manager .log_operation(
            "Recuperata configurazione email",
            current_user .get('username'))
        return response_manager .create_success_response(
...
@app .put("/api/mail/config",
          summary="🔒 Aggiorna configurazione email",
          tags=["Mail"])
...
def get_available_logs(
    ...
    try:
        ...
        log_manager .log_operation(
            "Recuperata lista log disponibili",
            current_user .get('username'))
        return response_manager .create_success_response(
        ...
        log_manager .log_operation(
            "Errore recupero log disponibili",
            current_user .get('username'),
            str(e))
        raise HTTPException(
            status_code=500,
            detail="Errore durante il recupero dei log disponibili")
...
def get_logs(
        log_type: str = Query(
            "npm_debug",
            description="Tipo di log da recuperare"),
        limit: Optional[int] = Query(
            1500,
            description="Numero massimo di log da ritornare (default: 1500)"),
        offset: int = Query(
            0,
            description="Numero di log da saltare (paginazione)"),
        search: Optional[str] = Query(
            None,
            description="Stringa di ricerca nei log"),
        *,
        request: Request,
        response: Response,
        credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    try:
        ...
        log_manager .log_operation(
            f"Errore recupero logs {log_type}",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(status_code=500,
                            detail="Errore durante il recupero dei logs")
...
def get_logs_stats(
    ...
    *,
    ...
    try:
        ...
        log_manager .log_operation(
            f"Recuperate statistiche log {log_type}",
            current_user .get('username'))
...
def search_logs(
    ...
    *,
    ...
    try:
        ...
        log_manager .log_operation(
            f"Errore ricerca nei logs {log_type}",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(status_code=500,
                            detail="Errore durante la ricerca nei logs")
...
def get_patterns(
    ...
    try:
        ...
        return response_manager .create_success_response(extra_fields={
            "patterns": patterns,
            "stats": stats
        })
    except Exception as e:
        log_manager .log_operation(
            "Errore recupero pattern",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(status_code=500,
                            detail="Errore durante il recupero dei pattern")


@app .post("/api/patterns", summary="🔒 Aggiunta pattern", tags=["Patterns"])
@handle_endpoint_exceptions("aggiunta pattern")
def add_pattern(pattern_type: str = Query(...,
    ...
    try:
        ...
        log_manager .log_operation(
            "Errore aggiunta pattern",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(status_code=500,
                            detail="Errore durante l'aggiunta del pattern")


@app .delete("/api/patterns/{pattern_type}/{pattern_id}",
             summary="🔒 Rimozione pattern", tags=["Patterns"])
...
def remove_pattern(
    ...
    *,
    ...
    try:
        ...
        log_manager .log_operation(
            "Errore rimozione pattern",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(status_code=500,
                            detail="Errore durante la rimozione del pattern")


@app .put("/api/patterns/{pattern_type}/{pattern_id}",
          summary="🔒 Aggiornamento pattern", tags=["Patterns"])
...
def update_pattern(
    ...
    *,
    ...
    try:
        ...
        log_manager .log_operation(
            "Errore aggiornamento pattern",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(
            status_code=500,
            detail="Errore durante l'aggiornamento del pattern")


@app .get("/api/patterns/stats",
          summary="🔒 Recupero statistiche pattern",
          tags=["Patterns"])
...
def get_pattern_stats(
    *,
    ...
    try:
        ...
        log_manager .log_operation(
            "Recuperate statistiche pattern",
            current_user .get('username')
        )
        ...
        log_manager .log_operation(
            "Errore recupero statistiche pattern",
            current_user .get('username'),
            str(e)
        )
        raise HTTPException(
...
async def get_secure_config(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore durante la lettura della configurazione: {
                str(e)}")
...
async def update_secure_config(
    ...
    try:
        ...
        if 'SECURE_COOKIES'not in config_data:
            raise HTTPException(
                status_code=400,
                detail="Campo SECURE_COOKIES mancante")
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Errore durante l'aggiornamento: {
                str(e)}")
...
@app .get("/api/services/restart-status/{service_name}",
...
async def get_restart_status(
    ...
    return response_manager .create_success_response(
        data=result,
        message="Stato del restart recuperato"
    )


@app .get("/api/services/pending-restarts",
