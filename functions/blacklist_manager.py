...
def load_blacklists_once(
    ...
    try:
        with open(user_agent_blacklist, "r")as ua_file:
            user_agents_blacklist = {line .strip().lower()
                                     for line in ua_file if line .strip()}
        debug_log(
            f"Blacklist User-Agent caricata: {len(user_agents_blacklist)} voci", npm_debug_log)
    ...
    try:
        with open(intent_blacklist, "r")as intent_file:
            intent_blacklist_set = {line .strip().lower()
                                    for line in intent_file if line .strip()}
        debug_log(
            f"Blacklist Intenti caricata: {
                len(intent_blacklist_set)} voci",
            npm_debug_log)
    except FileNotFoundError:
        intent_blacklist_set = set()
