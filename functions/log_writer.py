...
from .pattern_matcher import descrizione_intento, descrizione_user_agent, get_status_meaning
...
def write_log(
    ...
    ua_short = (user_agent[:max_ua_len] +
                '...')if user_agent and len(user_agent) > max_ua_len else user_agent
    url_short = (url[:max_url_len] +
                 '...')if url and len(url) > max_url_len else url
