...
def load_config(CONFIG_PATH, NPM_DEBUG_LOG):
    ...
    if not os .path .isfile(CONFIG_PATH):
        debug_log(
            f"[ERRORE] File config non trovato: {CONFIG_PATH}",
            NPM_DEBUG_LOG)
    ...
    if "CODES_TO_ALLOW" in config:
        if isinstance(config["CODES_TO_ALLOW"], list):
            config["CODES_TO_ALLOW"] = set(
                int(x)for x in config["CODES_TO_ALLOW"])
        else:
            debug_log(
                f"[ERRORE] 'CODES_TO_ALLOW' deve essere una lista",
                NPM_DEBUG_LOG)
    ...
    for int_key in ["TIME_FRAME", "MAX_REQUESTS"]:
        if int_key in config:
            try:
                ...
                debug_log(
                    f"[ERRORE] '{int_key}' deve essere un intero",
                    NPM_DEBUG_LOG)
    ...
    for key in REQUIRED_KEYS:
        if key not in config:
            debug_log(
                f"[ERRORE] Config: parametro mancante '{key}'",
                NPM_DEBUG_LOG)
            exit(
                f"[ERRORE FATALE] Parametro '{key}' mancante in {CONFIG_PATH}")
