...
def save_automatic_ban_to_db(
    ...
    try:
        ...
        if existing_ip:
            ...
            conn .commit()
            debug_log(
                f"Ban automatico per IP {ip} aggiunto alla tabella 'automatic_bans' con info geo: {
                    ip_info['organization']} ({
                    ip_info['country']}).",
                NPM_DEBUG_LOG)
        ...
        debug_log(
            f"Errore nel salvataggio del ban automatico per IP {ip}: {e}",
            NPM_DEBUG_LOG)
...
def ban_ip(ip, jail_name, NPM_DEBUG_LOG):
    ...
    try:
        ...
        if success:
            ...
            debug_log(
                f"IP: {ip}, Comando 'fail2ban-client' eseguito FALLITO. Return code: {
                    result .returncode}, stdout: {stdout}, stderr: {stderr}",
                NPM_DEBUG_LOG)
        ...
        debug_log(
            f"IP: {ip}, ERRORE nell'esecuzione del comando ban: {e}",
            NPM_DEBUG_LOG)
        return False
    ...
    if success:
        ...
        with ip_manager .ip_data_lock:
            if ip in ip_manager .ip_data:
                ...
                debug_log(
                    f"IP {ip} rimosso dalla memoria dopo il ban.",
                    NPM_DEBUG_LOG)
