...
class ServerManager:
    ...
    def check_restart_commands(self):
        services = ["backend", "analyzer"]
        if self .geolocate_enabled:
            services .append("geolocate")
        ...
        for service in services:
            command_file = os .path .join(
                self .service_commands_dir, f"{service}.restart")
    ...
    def stream_output(self, process, source):
        try:
            ...
            self .log_message(
                f"Errore durante la lettura dell'output: {e}", source)
    ...
    def start_npm_analyzer(self):
        self .log_message(
            "Avvio NGINX Shield (npm_analyzer.py)...",
            "ANALYZER")
                ...
                with open("log/geolocate.log", "a", encoding="utf-8")as f:
                    for line in iter(
                        ...
                        if line and self .running:
                            ...
                            if decoded_line:
                                timestamp = datetime .now().strftime("%Y-%m-%d %H:%M:%S")
                                log_entry = f"[{timestamp}] [GEOLOCATE] {decoded_line}\n"
                                f .write(log_entry)
                                print(log_entry .strip())
            except Exception as e:
                self .log_message(
                    f"Errore durante la lettura dell'output geolocate: {e}",
                    "GEOLOCATE")
        threading .Thread(target=stream_geolocate, daemon=True).start()
        self .log_message(
            "Geolocate service avviato con successo",
            "GEOLOCATE")
    ...
    def restart_analyzer(self):
        self .log_message(
            "Riavvio NGINX Shield (analyzer) in corso...",
            "ANALYZER")
        ...
        self .log_message(
            "NGINX Shield (analyzer) riavviato con successo",
            "ANALYZER")
    ...
    def restart_geolocate(self):
        ...
        self .log_message(
            "Geolocate service riavviato con successo",
            "GEOLOCATE")
    ...
    def signal_handler(self, signum, frame):
        self .log_message(
            f"Ricevuto segnale {signum}, arresto in corso...",
            "SYSTEM")
        self .stop_services()
        sys .exit(0)
    ...
    def run(self):
        ...
        if not os .path .exists("npm_analyzer.py"):
            self .log_message(
                "ERRORE: File 'npm_analyzer.py' non trovato", "SYSTEM")
            return False
        if not os .path .exists("backend"):
            self .log_message(
                "ERRORE: Directory 'backend' non trovata", "SYSTEM")
            return False
        if not os .path .exists("package.json"):
            self .log_message(
                "ERRORE: File 'package.json' non trovato", "SYSTEM")
            return False
        if self .geolocate_enabled and not os .path .exists("geolocate"):
            self .log_message(
                "ERRORE: Directory 'geolocate' non trovata", "SYSTEM")
            return False
        ...
        if self .geolocate_enabled:
            ...
            self .log_message(
                "Servizio di geolocalizzazione DISABILITATO (DISABLE_GEOLOCATE=true)",
                "SYSTEM")
        ...
        try:
            while self .running:
                ...
                if self .backend_process and self .backend_process .poll() is not None:
                    self .log_message(
                        "Backend terminato inaspettatamente", "BACKEND")
                    break
                if self .frontend_process and self .frontend_process .poll() is not None:
                    self .log_message(
                        "Frontend terminato inaspettatamente", "FRONTEND")
                    break
                if self .npm_analyzer_process and self .npm_analyzer_process .poll() is not None:
                    self .log_message(
                        "NGINX Shield (npm_analyzer) terminato inaspettatamente", "ANALYZER")
                    break
                if self .geolocate_enabled and self .geolocate_process and self .geolocate_process .poll() is not None:
                    self .log_message(
