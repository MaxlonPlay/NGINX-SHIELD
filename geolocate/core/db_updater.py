...
class DBUpdater:
    def __init__(self):
        self.data_filename = os.getenv("DATA_FILENAME", "./data/geoip/networks.csv")
                    ...
                    for chunk in r.iter_content(chunk_size=8192):
                        ...
                        if time.time() - start_time > 1:
                            if total_size > 0:
                                ...
                                print(
                                    f"\rScaricamento: {downloaded_size / (1024 * 1024):.2f}MB",
                                    end="",
                                )
                            start_time = time.time()
    ...
    def _validate_csv(self):
        ...
        try:
            with open(self.temp_filename, "r", encoding="utf-8") as f:
                ...
                if not all(
                    col in header
                    for col in ["network", "asn", "organization", "country"]
                ):
                    print(
    ...
    def _update_files(self):
        ...
        try:
            if os.path.exists(self.data_filename):
                ...
                print(f"   Rinominato '{self.data_filename}' in '{self.old_filename}'")
            shutil.move(self.temp_filename, self.data_filename)
            print(f"   Spostato '{self.temp_filename}' a '{self.data_filename}'")
    ...
    def _clear_cache(self):
        ...
        try:
            cache_files = list(
                self.cache_dir.glob(f"{Path(self .data_filename).stem}_daemon_v*.cache")
            )
            if not cache_files:
