    ...
    def do_GET(self):
        ...
        try:
            ...
            self._send_response(
                400,
                "application/json",
                json.dumps({"success": False, "error": "Invalid IP address format."}),
            )
            return
        start_time = time.time()
        matching_row = lookup_service.find_matching_cidr(ip_address_normalized)
        ...
        if matching_row:
            ...
            self._send_response(
                404, "application/json", json.dumps(response_data, indent=2)
            )
...
class GeoWebAPI:
    ...
    def start(self):
        print(
            f"[INFO] Avvio Web API server su http://{self.daemon_web_host}:{self.daemon_web_port}"
        )
        print(
            f"[INFO] Esempio d'uso http://{self.daemon_web_host}:{self.daemon_web_port}/216.58.205.46"
        )
        ...
        try:
            ...
            print(
                f"[ERROR] Errore Web API server: {e} - Probabilmente la porta {self.daemon_web_port} è già in uso."
            )
            raise
